...
corrupted:
...
#if 1 //add by [francis],20180927,add backup check point
	...
	if(1 == c->vi.ubi_num)
	{
		Quectel_Set_Partition_RestoreFlag("modem",10);
	}
	...
	if( 0 == c->vi.ubi_num)
	{
		...
		{
			printk("@Ramos set restore systemfs flag here 101010\r\n");
			Quectel_Set_Partition_RestoreFlag("system",10);
		}
		else
		{
	
			Quectel_Set_Partition_RestoreFlag("recoveryfs",10);
		}
	}
#endif
