...
st_api_test_case at_sim_testlist[] = 
{
    ...
    {17,   "QL_MCM_SIM_OpenLogicalChannel"},
    {18,   "QL_MCM_SIM_CloseLogicalChannel"},
    {19,   "QL_MCM_SIM_SendApdu\n"},
...
static void print_hexstr(uint8_t *apdu,uint32_t apdu_len)
{
    int i = 0;
...
static uint8_t chrtodec(char chr)
{
    uint8_t value=0;
...
static int cal16match(uint8_t n)
{
    uint8_t i=0;
    int value=1;
...
static int hexstrto_dec(char *chr)
{
    uint8_t i,chrlen;
    uint8_t sta=0; 
    int chrtoint=0;
...
static bool long_hexstr_to_oct
(
    uint8_t     *pSrc,
    uint32_t    uSrcLen,
    uint8_t     *pDest,
...
static int test_sim(void)
{
    ...
    while(1)
    {
        ...
        case 17:    // "QL_MCM_SIM_OpenLogicalChannel"
        {
            QL_MCM_SIM_AID_INFO_T sAidInfo;
            ...
            QL_MCM_SIM_LOGICAL_CHANNEL_T sOpenLogcalChannelResp;

            ret = QL_MCM_SIM_OpenLogicalChannel(h_sim,E_QL_MCM_SIM_SLOT_ID_1,&sAidInfo,&sOpenLogcalChannelResp);
            if(ret != E_QL_SUCCESS)
            {
                printf("QL_MCM_SIM_OpenLogicalChannel FAIL.	ret:%d\n",ret);
                break;
            }
        ...
        case 18:    // "QL_MCM_SIM_CloseLogicalChannel"
        {
            ret = QL_MCM_SIM_CloseLogicalChannel(h_sim, E_QL_MCM_SIM_SLOT_ID_1, channel_id);
            if(ret != E_QL_SUCCESS)
            {
        ...
        case 19:    // "QL_MCM_SIM_SendApdu"
        {
            QL_MCM_SIM_SEND_APDU_T *p_sApdu = NULL;
            QL_MCM_SIM_SEND_APDU_RESPONSE_T *p_sSendApduResp = NULL;

            p_sApdu = (QL_MCM_SIM_SEND_APDU_T*)malloc(sizeof(QL_MCM_SIM_SEND_APDU_T));
            if (NULL == p_sApdu)
            {
                printf("failed to malloc p_sApdu\n");
                break;
            }

            p_sSendApduResp = (QL_MCM_SIM_SEND_APDU_RESPONSE_T*)malloc(sizeof(QL_MCM_SIM_SEND_APDU_RESPONSE_T));
            if (NULL == p_sSendApduResp)
            {
                printf("failed to malloc p_sSendApduResp\n");
                free(p_sApdu);
                break;
            }
            ...
            memset(p_sApdu,0,sizeof(*p_sApdu));
            memset(p_sSendApduResp,0,sizeof(*p_sSendApduResp));

            if(false == long_hexstr_to_oct(aApdu,strlen(aApdu),p_sApdu->apdu,&p_sApdu->apdu_len,sizeof(p_sApdu->apdu)))
            {
                printf(" long_hexstr_to_oct FAIL\n");
                free(p_sApdu);
                free(p_sSendApduResp);
                break; 
            }

            p_sApdu->channel_id = channel_id;
            ret = QL_MCM_SIM_SendApdu(h_sim, E_QL_MCM_SIM_SLOT_ID_1, p_sApdu, p_sSendApduResp);
            if(ret != E_QL_SUCCESS)
            {
                printf("QL_MCM_SIM_SendApdu FAIL, ret:%d\n",ret);
                free(p_sApdu);
                free(p_sSendApduResp);
                break;
            }
            printf("QL_MCM_SIM_SendApdu ret:%d\n",ret);

            if(p_sSendApduResp->apdu_valid)
            {
                print_hexstr(p_sSendApduResp->apdu,p_sSendApduResp->apdu_len);
            }
            free(p_sApdu);
            free(p_sSendApduResp);
            break;
