...
static int test_nw(void)
{
    ...
    while(1)
    {
        ...
        {
           int i = 0;
           unsigned short mcc=0, mnc=0;
           QL_MCM_NW_CELL_INFO_T    t_info;
           memset(&t_info, 0, sizeof(QL_MCM_NW_CELL_INFO_T));
           ret = QL_MCM_NW_GetCellInfo(h_nw, &t_info);  
           printf("QL_MCM_NW_GetCellInfo, ret=%d, detail info:\n", ret);
           ...
           if(t_info.gsm_info_valid)
           {
               ...
               for(i = 0; i < t_info.gsm_info_len; i++)
               {
                   ...
                   get_mcc_mnc_value(t_info.gsm_info[i].plmn, 3, &mcc, &mnc);
           ...
           if(t_info.umts_info_valid)
           {
               ...
               for(i = 0; i < t_info.umts_info_len; i++)
               {
                   ...
                   get_mcc_mnc_value(t_info.umts_info[i].plmn, 3, &mcc, &mnc);
                   printf("convert plmn to mcc=%d,mnc=%02d\n", mcc, mnc);
               }
           }
           ...
           if(t_info.lte_info_valid)
           {
               ...
               for(i = 0; i < t_info.lte_info_len; i++)
               {
                   ...
                   get_mcc_mnc_value(t_info.lte_info[i].plmn, 3, &mcc, &mnc);
                   printf("convert plmn to mcc=%d,mnc=%02d\n", mcc, mnc);
               }
           }
