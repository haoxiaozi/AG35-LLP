...
{
        char mode_spilt[128] = {0};
	if(mode > 0)
	{
		if(mode & QL_MCM_NW_MODE_GSM)
		{
			strcat(mode_spilt,"GSM ");
		}
		if(mode & QL_MCM_NW_MODE_WCDMA)
		{
			strcat(mode_spilt,"WCDMA ");
		}
		if(mode & QL_MCM_NW_MODE_CDMA)
		{
			strcat(mode_spilt,"CDMA ");
		}
		if(mode & QL_MCM_NW_MODE_EVDO)
		{
			strcat(mode_spilt,"EVDO ");
		}
		if(mode & QL_MCM_NW_MODE_LTE)
		{
			strcat(mode_spilt,"LTE ");
		}
		if(mode & QL_MCM_NW_MODE_TDSCDMA)
		{
			strcat(mode_spilt,"TDSCDMA ");
		}
		if(mode & QL_MCM_NW_MODE_PRL)
		{
			strcat(mode_spilt,"PRL ");
		}
		printFormatMsg(" nw_mode split as: %s\n", mode_spilt);
	}
}
/********************************TEST CASE START******************************/
...
static void TEST_QL_MCM_NW_GetOperatorName(void)
{
	...
	{	
	    ...
	    switch(mnc)
	    {
	        case 03:
	        case 05:
	        case 11: sim_format = 0;  break;/*CDMA*/
	        case 00:
	        case 01:
	        case 02:
	        case 06:
	        case 07:
	        case 20: sim_format = 1;  break; /*GSM*/
	        default: sim_format = -1; break;
		}
	}
	printFormatMsg("long_eons:%s short_eons:%s mcc:%s mnc:%s sim_format:%d(0-CDMA 1-GSM)\n",
							info.long_eons, info.short_eons, info.mcc, info.mnc, sim_format);
	return;
}

int QL_MCM_NW_GetSIMFormat(void)
{
    return sim_format;
}
/**
...
static void TEST_QL_MCM_NW_GetRegStatus(void)
{
	...
	printFormatMsg("voice_registration:tech_domain:%s, radio_tech:%s, roaming:%s, state:%s\n",
	...
	printFormatMsg("data_registration:tech_domain:%s, radio_tech:%s, roaming:%s, state:%s\n",
...
//QL_MCM_NW_SetConfig
static void TEST_QL_MCM_NW_SetConfig(void)
{
    int ret;
    QL_MCM_NW_CONFIG_INFO_T info = {0};
    ...
    info.preferred_nw_mode = 0x3F;
    info.roaming_pref = 1;
    ret = QL_MCM_NW_SetConfig(h_nw, &info);

    CU_ASSERT_EQUAL(ret, MCM_SUCCESS_V01);
    if(ret != MCM_SUCCESS_V01)
    {
        printFormatMsg("QL_MCM_NW_SetConfig failed.ret=%d\n", ret);
        return;
    }

    printFormatMsg("QL_MCM_NW_SetConfig: %lld\n", info.preferred_nw_mode);

    return;
}

static void TEST_QL_MCM_NW_EventRegister(void)
{
    int ret;
    int mask = 0xFF;
    ...
    if(ret != MCM_SUCCESS_V01)
    {
        printFormatMsg("QL_MCM_NW_EventRegister failed.ret=%d\n", ret);
        return;
    }
    printFormatMsg("QL_MCM_NW_EventRegister successful.ret=%d\n", ret);
    return ;
}

static void TEST_QL_MCM_NW_SetLowPowerMode(void)
{
    int ret;
    int mode = 1;
    ...
    if(ret != MCM_SUCCESS_V01)
    {
        printFormatMsg("QL_MCM_NW_SetLowPowerMode failed.ret=%d\n", ret);
        return;
    }
    printFormatMsg("QL_MCM_NW_SetLowPowerMode successful.ret=%d\n", ret);
    return ;
}
...
static CU_TestInfo nw_suite_case[] = {
  {"QL_MCM_NW_GetConfig", TEST_QL_MCM_NW_GetConfig},
  {"QL_MCM_NW_GetNitzTimeInfo", TEST_QL_MCM_NW_GetNitzTimeInfo},
  {"QL_MCM_NW_GetOperatorName", TEST_QL_MCM_NW_GetOperatorName},
  {"QL_MCM_NW_GetRegStatus", TEST_QL_MCM_NW_GetRegStatus},
  {"QL_MCM_NW_GetSignalStrength", TEST_QL_MCM_NW_GetSignalStrength},
  {"QL_MCM_NW_GetCellAccessState", TEST_QL_MCM_NW_GetCellAccessState},
  {"QL_MCM_NW_SetConfig",TEST_QL_MCM_NW_SetConfig},
  {"QL_MCM_NW_EventRegister",TEST_QL_MCM_NW_EventRegister},
  {"QL_MCM_NW_SetLowPowerMode",TEST_QL_MCM_NW_SetLowPowerMode},
  CU_TEST_INFO_NULL,
};
...
CU_SuiteInfo nw_suites[] = {
	{"nw", nw_suite_init, nw_suite_clean, nw_suite_SetUp, nw_suite_TearDown, nw_suite_case},
	CU_SUITE_INFO_NULL,
};
