...
st_api_test_case at_api_testcases[] = 
{
    ...
    {46,    "QL_LOC_Set_Auto_Inject_Xtra"},
    {47,    "QL_LOC_Set_Delete_Aiding_Data\n"},
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        case 46://QL_LOC_AUTO_INJECT_XTRA
        {
            char injectStr[128]={0};
            printf("please input Y/N Enable or Disable Auto Inject Time And XTRA: \n");
            scanf("%s",injectStr);
            if(strcmp(injectStr,"Y")==0)
            {
                ret = QL_LOC_Set_Auto_Inject_Xtra(h_loc, 1);
                printf("QL_LOC_Set_Auto_Inject_Xtra ret=%d\n", ret);
            }
            else if(strcmp(injectStr,"N")==0)
            {
                ret = QL_LOC_Set_Auto_Inject_Xtra(h_loc, 0);
                printf("QL_LOC_Set_Auto_Inject_Xtra ret=%d\n", ret);
            }
            break;
        }
        case 47://QL_LOC_Delete_Aiding_Data
        {
            char injectStr[128]={0};
            printf("please input Y/N Delete or not all GPS assisted data: \n");
            scanf("%s",injectStr);
            if(strcmp(injectStr,"Y")==0)
            {
                ret = QL_LOC_Delete_Aiding_Data(h_loc, E_QL_LOC_DELETE_ALL);
